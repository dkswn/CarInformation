...
public class Benz extends WindowAdapter implements ActionListener{
	...
	private String[] images = { "image/eqa250.jpg", "image/EQAAMG.png", "image/EQAAMGplus.png",
			"image/EQB.png","image/EQC400.png", "image/EQS.png", "image/EQSedition.png"};
	...
	public Benz() {
		...
		Area.append("  넘버: " + arr.get(getIndex()).getNo() + "\n"+ "  모델: " + arr.get(getIndex()).getModel()
				...
				+ "\n" + "  1회 충전주행거리:" + arr.get(getIndex()).getMaxdistance() + "\n");
...
// frame 안에 이미지추가
		...
		JButton logo = new JButton(new ImageIcon("image\\벤츠로고.png"));
	...
	public ArrayList<CarDTO> BenzF() {
		...
		try {
			...
			sql.append(
					"select NO, BRAND, MODEL, PRICE, TYPE, BCAPACITY, POWER, MILEAGE, MAXDISTANCE from CAR where Brand = 'Benz' ");
